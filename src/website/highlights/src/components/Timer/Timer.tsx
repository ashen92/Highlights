...
const convertHighlightToTask = (highlight: HighlightTask): Task => {
  return {
    id: highlight.id,
    taskId: highlight.id.toString(), // Convert to string
    title: highlight.highlight_name, // Map highlight_name to title
    description: highlight.description || '', // Provide a default empty string if no description
    estimatedTime: highlight.estimated_time || null, 
    completed: false, // Default to false
    dueDate: highlight.due_date ? new Date(highlight.due_date) : null,
    Date: highlight.due_date || '', // Provide a default empty string
    startTime: '', // Default empty string
    endTime: '', // Default empty string
    reminder: '', // Default empty string
    priority: highlight.priority || 'medium', // Default to medium if no priority
    label: highlight.label || '', // Default to empty string
...
const HighlightMenu = ({ 
  ...
  highlights: Task[], 
  onHighlightSelect: (task: Task) => void, 
  closeMenu: () => void 
}) => {
...
const Timer: React.FC<TimerProps> = ({ onEndButtonClick, refreshTrigger }) => {
  ...
  const { highlights: rawHighlights, isHighlightsLoading, isHighlightsError } = useHighlights();
  ...
  useEffect(() => {
             ...
             const highlights: Task[] = rawHighlights 
             ? rawHighlights.map(convertHighlightToTask) 
             : [];
  ...
  const handleHighlightSelect = (task: Task) => {
      ...
      <div className={styles.pomodoro}>
        <div className={styles.focusLink}>
          <Menu withArrow opened={menuOpened} onChange={setMenuOpened}>
            <Menu.Target>
              <UserButton
                label={selectedTask ? selectedTask.title : "Focus"}
                styles={{
                  label: {
                    fontSize: '14px',
                  },
